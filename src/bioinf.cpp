    ...
    unordered_map<int64_t, pair<string, string>>& final_dict)
{
    ...
    for (const auto& [pos, votes] : dict) {
        ...
        if (votes.none >= votes.substituted && 
            votes.none >= votes.inserted && 
        ...
        while (i < record.cigar.length()) {
            ...
            if (op == 'M') {
                ...
                for (int j = 0; j < length; ++j) {
                    ...
                    matchingFile << "refpos "  << refPos << " - REF_BASE " << refBase << " | " << "readpos " << (readPos + j + 1) << " - READ_BASE " << readBase << " [INSERT]" << endl;
